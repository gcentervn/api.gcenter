<?php
class Games_categories extends Trongate {
    ...
    function create() {
        ...
        if ($update_id>0) {
            $data['headline'] = 'Update Games Category Record';
            $data['cancel_url'] = BASE_URL.'games_categories/show/'.$update_id;
        } else {
            $data['headline'] = 'Create New Games Category Record';
            $data['cancel_url'] = BASE_URL.'games_categories/manage';
        }

        $data['form_location'] = BASE_URL.'games_categories/submit/'.$update_id;
    ...
    function manage() {
        ...
        if (segment(4) !== '') {
            ...
            $sql = 'select * from games_categories
            WHERE name LIKE :name
            OR description LIKE :description
            ORDER BY id';
            $all_rows = $this->model->query_bind($sql, $params, 'object');
        } else {
            $data['headline'] = 'Manage Games Categories';
        ...
        $pagination_data['pagination_root'] = 'games_categories/manage';
        $pagination_data['record_name_plural'] = 'games categories';
        ...
        $data['view_module'] = 'games_categories';
    ...
    function show() {
        ...
        if ($update_id == 0) {
            redirect('games_categories/manage');
        }
        ...
        if ($data == false) {
            ...
            $data['headline'] = 'Games Category Information';
            $data['view_file'] = 'show';
            $this->template('admin', $data);
        }
    }
        ...
        if ($submit == 'Submit') {
            ...
            if ($result == true) {
                ...
                if ($update_id>0) {
                    ...
                    $this->model->update($update_id, $data, 'games_categories');
                    $flash_msg = 'The record was successfully updated';
                } else {
                    //insert the new record
                    $update_id = $this->model->insert($data, 'games_categories');
                    $flash_msg = 'The record was successfully created';
                }

                set_flashdata($flash_msg);
                redirect('games_categories/show/'.$update_id);
    ...
    function submit_delete() {
        ...
        if (($submit == 'Yes - Delete Now') && ($params['update_id']>0)) {
            ...
            $params['module'] = 'games_categories';
            $this->model->query_bind($sql, $params);

            //delete the record
            $this->model->delete($params['update_id'], 'games_categories');
    ...
    function _get_data_from_db($update_id) {
        $record_obj = $this->model->get_where($update_id, 'games_categories');
    ...
    function _init_picture_settings() { 
        ...
        $picture_settings['destination'] = 'games_categories_pics';
        $picture_settings['target_column_name'] = 'picture';
        $picture_settings['thumbnail_dir'] = 'games_categories_pics_thumbnails';
